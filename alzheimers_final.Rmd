---
title: "Final Project: Alzheimers"
...
# eTIV distribution
boxplot(alzheimers$eTIV, main="Estimated total intracranial volume (mm^3)", xlab= "eTIV", col = "aliceblue")
# relationship between eTIV and nWBV
qqplot(alzheimers$nWBV,alzheimers$eTIV, ylab = "Estimated total intracranial volume", xlab="Normalized whole brain volume", main = "eTIV vs nWBV", col = "lightsalmon1")
# correlation between Social Economic Status and Education
scatter.smooth(alzheimers$SES, alzheimers$EDUC, main= "Social economics & Education", xlab= "Social econic status (1= High, 5= low)", ylab = "Years of education", col = "plum2")
...
hist(alzheimers$SES, main = "Socioeconomic Status Distribution", xlab = "SES", col = "gold")
hist(alzheimers$MMSE, main = "MMSE Distribution", xlab = "MMSE", col = "deeppink")
hist(alzheimers$nWBV, main = "nWBV Distribution", xlab = "nWBV", col = "purple")
hist(alzheimers$ASF, main = "ASF Distribution", xlab = "ASF", col = "tomato")
plot(as.factor(alzheimers$Dement), main = "Demented vs Non Demented", xlab = "Demented (0 = nondemented, 1 = demented)", col = "maroon")
plot(as.factor(alzheimers$Sex), main = "Sex Distribution", xlab = "Sex (0 = Female, 1 = Male)", col= "steelblue")
plot(alzheimers)
```
...
Under the coefficients are parameters for the model. The estimate value (under the intercept estimate) of the logistic regression provides the log of odd ratio. The intercept estimates 65.082 is the null model value when all the variables is zero. For every one unit changes in any of the variables (Age, MMSE, nWBV, Sex, EDUC), the log of odd increases or decreases according to Estimate values. For the final model, age will decrease the log of odd ratio by -0.1556 with one unit increases in age, MMSE will decrease the log odds by -1.2088 with one unit increase in MMSE, nWBV will decrease the log odds by -24.31 with one unit increases in nWBV, being male increases the log odds by 0.9068, and EDUC will decrease the log odds by -0.153 with one unit increase in years of education. The standard error is the estimated variability due to sampling variability. In this case, the standard error for Age is 0.04 and nWBV is 7.37. According to the p values and significant codes, Age, MMSE and nWBV has strong association with Dement than Sex variable. EDUC is not significant according to the p-values and significant codes.
...
logi.hist.plot(alzheimers$EDUC,alzheimers$Dement, boxp=FALSE, type= "histogram", col="gray", main = "Demented vs EDUC")
logi.hist.plot(alzheimers$Sex,alzheimers$Dement, boxp=FALSE, type= "histogram", col="gray", main = "Demented vs Sex")
```
The logistic curve with histogram of each variable of the final model and demented is created above to see the probablity of being demented.
```{r}
...
The above equation is generated from model.final output and  exponential coefficient of the model.final is calculated to figure out by what factor each coefficient increases with one unit increases in independent variable.
...
alz.org
Brain Tour. (n.d). Retrieved Dec 10, 2017, from Alzheimer's association website : http://www.alz.org/
...
OASIS: Longitudinal MRI Data in Nondemented and Demented Older Adults. (n.d). Retrieved Dec 6, 2017 from Open Access Series of Imaging Studies (OASIS) website : http://www.oasis-brains.org/
